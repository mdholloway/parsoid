...
function DOMPostProcessor(env, options) {
	...
	this.processors = [];
	var addPP = (name, proc, skipNested) => {
	...
	addPP('normalize', (body) => { body.normalize(); });
	...
	addPP('handleEmptyElts', domVisitor.traverse.bind(domVisitor), true);
	...
	addPP('(li+table)Fixups+headings', domVisitor.traverse.bind(domVisitor), true);

	// Add <section> wrappers around sections
	addPP('wrapSections', wrapSections, true);
	...
	addPP('heading id uniqueness', domVisitor.traverse.bind(domVisitor), true);

	// Language conversion
	addPP('LanguageConverter', function(rootNode, env, options) {
		if (env.langConverterEnabled() && env.variantLanguage &&
			env.conf.wiki.variants.has(env.variantLanguage)) {
			LanguageConverter.baseToVariant(env, rootNode);
		}
	});

	if (env.conf.parsoid.linting) {
		addPP('linter', linter, true);
	}
	...
	addPP('addExtLinkClasses', function(rootNode, env, options) {
		return addExtLinkClasses(env, rootNode.ownerDocument);
	}, true);

	// (Optional) red links
	addPP('addRedLinks', function(rootNode, env, options) {
...
DOMPostProcessor.addMetaData = function(env, document) {
	...
	var styleURI = env.getModulesLoadURI() +
		'?modules=' + encodeURIComponent(Array.from(modules).join('|')) + '&only=styles&skin=minerva';
	appendToHead(document, 'link', { rel: 'stylesheet', href: styleURI });

	var siteStyleURI = env.scriptPath() + '/load.php?modules=mobile.site&only=styles&skin=minerva';
	appendToHead(document, 'link', { rel: 'stylesheet', href: siteStyleURI });
	...
	var src =  env.getModulesLoadURI() + '?modules=html5shiv&only=scripts&skin=minerva&sync=1';
